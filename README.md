...
`server.api_prefix` 只表示部署路径前缀，会拼接在现有 `/mockserver/...` 路由之前；例如配置为 `/tenant-a` 后，debug 登录接口会变成 `/tenant-a/mockserver/api/v1/auth/debug/login`。前端可通过 `VITE_API_PREFIX=/tenant-a` 使用同一个前缀；如果使用 `VITE_API_BASE_URL=/tenant-a`，开发代理也会代理 `/tenant-a/mockserver`。SDK 可通过 `MOCKSERVER_HOST=http://127.0.0.1:8080/tenant-a`，或 `MOCKSERVER_HOST=http://127.0.0.1:8080` + `MOCKSERVER_API_PREFIX=/tenant-a` 使用同一个前缀。静态部署时也可以在页面加载前设置 `window.__MOCKSERVER_API_PREFIX__ = "/tenant-a"`。
